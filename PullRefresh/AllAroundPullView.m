...
- (id)initWithScrollView:(UIScrollView *)scroll position:(AllAroundPullViewPosition)position {
    ...
    switch (position) {
        ...
        case AllAroundPullViewPositionBottom:
            offset = (position == AllAroundPullViewPositionBottom) ? (scroll.contentSize.height<scroll.bounds.size.height?scroll.bounds.size.height:scroll.contentSize.height) : 0.0f - scroll.bounds.size.height;
        ...
        case AllAroundPullViewPositionRight:
            offset = (position == AllAroundPullViewPositionLeft) ? -kSidePullViewWidth : (scroll.contentSize.width<scroll.bounds.size.width?scroll.bounds.size.width:scroll.contentSize.width);
...
- (void)updatePositionWhenContentsSizeIsChanged {
    if (self.position == AllAroundPullViewPositionBottom)
        if (self.scrollView.contentSize.height<self.scrollView.bounds.size.height) {
            self.frame = CGRectMake(0.0f, self.scrollView.bounds.size.height, self.frame.size.width, self.frame.size.height);
        } else {
            self.frame = CGRectMake(0.0f, self.scrollView.contentSize.height, self.frame.size.width, self.frame.size.height);
        }
        
    else if (self.position == AllAroundPullViewPositionRight) {
        CGRect rect = self.frame;
        if (self.scrollView.contentSize.width<self.scrollView.bounds.size.width) {
            rect.origin.x = self.scrollView.bounds.size.width;
        } else {
...
- (void)dealloc {
    ...
    self.allAroundPullViewActionHandler=nil;
}
